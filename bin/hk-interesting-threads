...
BORING_REGEXES = [
  re.compile("""
"(?:PRI )?IPC Server handler \d+ on \d+" daemon prio=\d+ tid=\S+ nid=\S+ waiting on condition \[\S+\]
...
\s+at java.util.concurrent.LinkedBlockingQueue.take\(LinkedBlockingQueue.java:\d+\)
...
""".strip()),
  re.compile("""
"[^"]+" daemon prio=\d+ tid=\S+ nid=\S+ waiting on condition \[\S+\]
\s+java.lang.Thread.State: WAITING \(parking\)
\s+at sun.misc.Unsafe.park\(Native Method\)
\s+- parking to wait for  <\S+> \(a java.util.concurrent.locks.AbstractQueuedSynchronizer\$ConditionObject\)
\s+at java.util.concurrent.locks.LockSupport.park\(LockSupport.java:\d+\)
\s+at java.util.concurrent.locks.AbstractQueuedSynchronizer\$ConditionObject.await\(AbstractQueuedSynchronizer.java:\d+\)
\s+at java.util.concurrent.LinkedBlockingQueue.take\(LinkedBlockingQueue.java:\d+\)
\s+at java.util.concurrent.ThreadPoolExecutor.getTask\(ThreadPoolExecutor.java:\d+\)
\s+at java.util.concurrent.ThreadPoolExecutor\$Worker.run\(ThreadPoolExecutor.java:\d+\)
\s+at java.lang.Thread.run\(Thread.java:\d+\)
""".strip()),
  re.compile("""
"IPC Reader \d+" prio=\d+ tid=\S+ nid=\S+ runnable \[\S+\]
\s+java.lang.Thread.State: .*
\s+at sun.nio.ch.EPollArrayWrapper.epollWait\(Native Method\)
\s+at sun.nio.ch.EPollArrayWrapper.poll\(EPollArrayWrapper.java:\d+\)
\s+at sun.nio.ch.EPollSelectorImpl.doSelect\(EPollSelectorImpl.java:\d+\)
\s+at sun.nio.ch.SelectorImpl.lockAndDoSelect\(SelectorImpl.java:\d+\)
\s+- locked <\S+> \(a sun.nio.ch.Util\$2\)
\s+- locked <\S+> \(a java.util.Collections\$UnmodifiableSet\)
\s+- locked <\S+> \(a sun.nio.ch.EPollSelectorImpl\)
\s+at sun.nio.ch.SelectorImpl.select\(SelectorImpl.java:\d+\)
\s+at sun.nio.ch.SelectorImpl.select\(SelectorImpl.java:\d+\)
\s+at org.apache.hadoop.hbase.ipc.HBaseServer\$Listener\$Reader.run\(HBaseServer.java:\d+\)
\s+- locked <\S+> \(a org.apache.hadoop.hbase.ipc.HBaseServer\$Listener\$Reader\)
\s+at java.util.concurrent.ThreadPoolExecutor\$Worker.runTask\(ThreadPoolExecutor.java:\d+\)
\s+at java.util.concurrent.ThreadPoolExecutor\$Worker.run\(ThreadPoolExecutor.java:\d+\)
\s+at java.lang.Thread.run\(Thread.java:\d+\)
""".strip()),
  re.compile("""
